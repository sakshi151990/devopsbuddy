...
import com.devopsbuddy.service.PlanService;
import com.devopsbuddy.service.S3Service;
import com.devopsbuddy.service.StripeService;
...
public class SignUpController {
	...
	public String signUpPost(@RequestParam(name = "planId", required = true) int planId,
			...
			throws IOException, S3Exception, InvalidRequestException {
		...
		User user = UserUtil.fromWebUserToDomainUser(payload);
		...
		if (file != null && !file.isEmpty()) {
			...
			if (profileImageUrl != null) {
				user.setProfile_img(profileImageUrl);
			} else {
